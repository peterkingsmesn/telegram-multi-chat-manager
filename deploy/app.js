...
// API 기본 URL
const API_BASE_URL = config.server.baseUrl + '/api';
...
function getKnownFirepowerAccounts() {
    // 화력 계정 목록은 서버에서 관리
    // 로컬 스토리지에 캐시된 데이터 사용
    const cached = localStorage.getItem('firepowerPhones');
    if (cached) {
        try {
            return JSON.parse(cached);
        } catch (e) {
            console.error('Failed to parse cached firepower phones:', e);
        }
    }
    
    // 캐시가 없으면 빈 배열 반환 (서버에서 로드 필요)
    return [];
}
...
async function reconnectMissingAccounts() {
    const criticalAccounts = await getCriticalAccounts(); // 서버에서 중요 계정 목록 가져오기
...
function showFirepowerApiModal(firepower) {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <input type="tel" id="firepowerPhoneInput" placeholder="전화번호 (예: +1234567890)">
...
// 자동 설정 전화번호는 서버에서 가져옴
let AUTO_SETUP_PHONES = [];
...
async function loadAutoSetupPhones() {
    try {
        ...
        console.error('❌ 자동 설정 전화번호 로드 실패:', error);
        AUTO_SETUP_PHONES = [];
    }
}
...
async function getCriticalAccounts() {
    try {
        ...
        console.error('❌ 중요 계정 목록 로드 실패:', error);
    }
    return [];
}
...
async function registerUserAPI() {
    ...
    if (!phone.startsWith('+') || phone.length < 7) {
        showRegistrationStatus('올바른 전화번호 형식이 아닙니다. (예: +1234567890, +33612345678, +81901234567)', 'error');
        return;
    }
